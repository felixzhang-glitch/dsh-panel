...
- 版本：验证版本 DSH 0.1.0-rc.6（token-usage）/ rc.8（time-awareness、better-sidebar 0.18.0）

### 关键用法

- host 侧：运行 `bunx @deepseek-ai/dsh@latest web`（dspm 拉起时追加 `--no-open`）；服务生命周期由 `dspm web <status|start|stop|restart|log>` 管理，`--restart` 复用同一 stop/start（日志 `~/.dsh/dsh-web.log`）
